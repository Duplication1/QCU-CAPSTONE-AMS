...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax']) && $_POST['ajax'] === '1') {
    ...
    if ($action === 'update_asset_tag') {
        ...
        try {
            ...
            // Get old asset tag and asset info (ensure it belongs to this room)
            $select_stmt = $conn->prepare("SELECT asset_tag, asset_name FROM assets WHERE id = ? AND room_id = ?");
            $select_stmt->bind_param('ii', $asset_id, $room_id);
            $select_stmt->execute();
            ...
            if (!$asset) {
                throw new Exception("Asset not found in this room");
            }
            ...
            // Update asset tag
            $update_stmt = $conn->prepare("UPDATE assets SET asset_tag = ? WHERE id = ? AND room_id = ?");
            $update_stmt->bind_param('sii', $new_asset_tag, $asset_id, $room_id);
            ...
            if ($success) {
                ...
                try {
                    ActivityLog::record(
                        ...
                        'update',
                        'asset',
                        "Updated asset tag from {$old_asset_tag} to {$new_asset_tag} for {$asset['asset_name']} in room {$room['name']}"
                    );
                } catch (Exception $e) {
...
if (empty($asset_ids_param)) {
    header("Location: roomassets.php?room_id=" . $room_id);
    exit();
}
...
// Fetch assets (only from this room)
...
$query = "SELECT a.id, a.asset_tag, a.asset_name, a.brand, a.model, a.serial_number, a.status, a.condition, r.name as room_name 
          ...
          WHERE a.id IN ($placeholders) AND a.room_id = ?
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6 px-6 py-4">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-800">Edit Asset Tags</h1>
                    <p class="text-sm text-gray-600 mt-1">
                        Update asset tags for <?php echo count($assets); ?> selected asset(s) in 
                        <span class="font-semibold"><?php echo htmlspecialchars($room['name']); ?></span>
                    </p>
                </div>
                <a href="roomassets.php?room_id=<?php echo $room_id; ?>" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors">
                    <i class="fa-solid fa-arrow-left mr-2"></i>Back to Room Assets
                </a>
            </div>
        </div>
                    ...
                    <div class="flex-1">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-700"><?php echo htmlspecialchars($asset['room_name'] ?: 'No Room'); ?></p>
                            </div>
                            <div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mt-6 p-4">
            <div class="flex items-center justify-between">
                ...
                <a href="roomassets.php?room_id=<?php echo $room_id; ?>" class="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors font-medium">
